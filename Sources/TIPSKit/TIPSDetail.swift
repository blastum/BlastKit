...
public struct TIPSDetail: Decodable {
    ...
    public static func mockWithIndexRatio(_ ratio: Double, for date: Date) -> TIPSDetail {
        // Create a mock instance with just the index ratio
        let json = """
        {
            "cusip": "MOCK",
            "index_date": "\(DateFormatter.iso8601.string(from: date))",
            "index_ratio": "\(ratio)"
        }
        """.data(using: .utf8)!
        ...
        return try! decoder.decode(TIPSDetail.self, from: json)
    }
}
