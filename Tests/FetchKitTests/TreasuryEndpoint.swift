...
public enum TreasuryEndpoint: Endpoint {
    ...
    public func decode(_ result: Result<(Data, URLResponse), any Error>) ->
        ...
        case let .success((data, _)):
            do {
                // Try to decode as Security first
                if let security = try? JSONDecoder().decode(Security.self, from: data) {
                    return .success(.security(security))
                }
                // If that fails, try to decode as Family
                if let family = try? JSONDecoder().decode(Family.self, from: data) {
                ...
                throw DecodingError.dataCorrupted(
                    DecodingError.Context(
                        codingPath: [],
                        debugDescription: "Could not decode as Security or Family"
                    )
                )
            } catch {
